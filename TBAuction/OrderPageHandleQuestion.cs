...
namespace TBAuction
{
    class OrderPageHandleQuestion
    {
        ...
        private void GetOrderFormContent(string orderPage)
        {
            ...
            MatchCollection matches = Regex.Matches(orderPage, "<form\\s+name\\s*=\\s*\"nameform\"\\s+.*?>.*?<\\s*/\\s*form\\s*>", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);
            if (matches.Count > 0)
            {
        ...
        #region 该段代码用于读取订单页面信息，目前可能在这种情况下不需要
        ...
        //获取促销信息的post字符串，如果isMax为true，则扫描筛选最佳的优惠方式，如果没有促销信息，返回string.Empty，目前可能不需要
        ...
        //获取店铺促销信息的post字符串，其实与普通促销信息差不多，出于淘宝升级的考虑单列，目前可能不需要
            ...
            foreach (InputControl inputControl in inputControls)
            {
                if (inputControl.Control.ToLower() == "input")
                {
                    if (inputControl.Type.ToLower() == "text")
                    {
                        ...
                        if (inputControl.Name.ToLower() == "_._0.c")
                        {
                            if (isCheckCode)
                            {
                                ...
                                Bitmap image = new Bitmap(auctionOperations.GetCheckCodeImageStream("http://checkcode.taobao.com/auction/checkcode?sessionID=" + encryptString, cc, out ccReturned));
                                cc.Add(ccReturned);
        ...
        public string GetOrderActionAddress(string orderPage)
        {
            ...
            Match match = Regex.Match(orderPage, "<form\\s+name\\s*=\\s*\"nameform\"\\s+.*?action\\s*=\\s*\"(?<action>.+?)\"", RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.Singleline);
            if (match.Success)
            {
                auctionLog.Log("通知：提取了答题秒杀订单页面的action地址：" + match.Groups["action"].ToString());
                return match.Groups["action"].ToString();
            }

            auctionLog.Log("警告：提取答题秒杀订单页面action地址失败");
            return string.Empty;
        }
        #endregion
